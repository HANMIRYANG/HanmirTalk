...
-- Creates the initial Hanmir Talk schema. Equivalent to applying
-- `server/src/db/schema.sql` against an empty database.
--
-- Apply with:
--   psql "$DATABASE_URL" -f server/src/db/migrations/001_initial.sql
--
-- The migration is intentionally idempotent (`IF NOT EXISTS`) so it can be
-- re-run safely against partially-bootstrapped databases.
